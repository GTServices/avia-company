...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.translates.store') }}" novalidate>
        ...
        <!-- Key Field -->
        <div class="mb-3">
            <label for="key" class="form-label">Ключ перевода</label>
        ...
        <div class="tab-content">
            @foreach($languages as $language)
                <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="tab-{{ $language->lang_code }}">
                    <div class="mb-3">
                        <label for="translation-{{ $language->lang_code }}" class="form-label">Перевод для {{ strtoupper($language->lang_code) }}</label>
                        <input type="text" name="translations[{{ $language->lang_code }}]" id="translation-{{ $language->lang_code }}" class="form-control" placeholder="Введите перевод для {{ strtoupper($language->lang_code) }}" required>
