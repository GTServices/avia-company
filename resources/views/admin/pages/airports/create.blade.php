...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.airports.store') }}" enctype="multipart/form-data">
        @csrf
        ...
        <div class="mb-3">
            ...
            <select name="city_id" id="city_id" class="form-control" required>
                @foreach($cities as $city)
                    <option value="{{ $city->id }}">{{ $city->getTranslation('name', 'ru') }}</option>
                @endforeach
            </select>
        </div>
            ...
            @foreach($languages as $language)
                <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="tab-{{ $language->lang_code }}">
                    <div class="mb-3">
                        ...
                        <input type="text" name="name[{{ $language->lang_code }}]" id="name-{{ $language->lang_code }}" class="form-control" placeholder="Введите название аэропорта на {{ strtoupper($language->lang_code) }}" required>
