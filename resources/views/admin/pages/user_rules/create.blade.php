...
@section('content')
    <div class="page-breadcrumb d-flex align-items-center mb-3">
        <div class="breadcrumb-title pe-3">Добавить новые правила</div>
    </div>

    <form method="POST" action="{{ route('admin.user_rules.store') }}" novalidate>
        @csrf
            ...
            @foreach($languages as $language)
                <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="tab-{{ $language->lang_code }}">
                    <div class="mb-3">
                        ...
                        <textarea name="rules[{{ $language->lang_code }}]" id="rules-{{ $language->lang_code }}" class="form-control ckeditor" placeholder="Введите правила на {{ strtoupper($language->lang_code) }}" rows="8"></textarea>
